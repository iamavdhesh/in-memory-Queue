...
public class JsonMessageQueue implements Queue {
    ...
    /*
        Add Message to Queue with its unique messageId for which the consumers have subscribed for.
        Convert the Message to Json and add to Queue
     */
    @Override
    public void addMessage(Message message, String messageId) {
        try {
            String queueMessage = getJsonMessage(message, messageId);
            inMemoryQueue.put(queueMessage);
            System.out.println("Produced Message - " + queueMessage);
            startConsumerThread();
        } catch (InterruptedException | IOException e) {
            System.err.println("Exception occurred during addition of message with messageId -" + messageId + " to queue, discarding message");
        }
    }
    ...
    private String getJsonMessage(Message message, String messageId) throws IOException {
        QueueMessage queueMessage = new QueueMessage(messageId, RETRY_COUNT, message);
        return objectMapper.writeValueAsString(queueMessage);
    }
    ...
    private void consume() {
        ...
        try {
            ...
            final String messageId = queueMessage.getMessageId();
            final Message message = queueMessage.getPayload();
            MessageSubscribersDetails messageSubscribersDetails = messagesSubscribers.get(messageId);
            //5. Check if subscriber list for this message is empty, if yes, then throw NoSubscriberFoundException
            checkIfAnyConsumerHasSubscribedToMessage(messageId, messageSubscribersDetails);
            ...
            for (Map.Entry<Consumer, String> subscribedConsumer : messageSubscriber.getConsumerDetails().entrySet()) {
                ...
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        consume(message, messageId, consumer, messageSubscriber);
                    }
                }).start();
            }
    ...
    private void consume(Message message, String messageId, Consumer consumer, MessageSubscribersDetails messageSubscribersDetails) {
        ...
        synchronized (subscribedConsumers) {
            try {
                ...
                try {
                    ...
                    System.err.println("Exception occurred during consumption of messageFromQueue - " + messageId +
                            " Cannot find callback method " + callBackMethod + " for consumer - " + consumer.getConsumerName());
                } catch (Exception e) {
                    System.err.println("Exception occurred during consumption of messageFromQueue for " + consumer.getConsumerName() + " for message " + messageId);
                }
            } catch (InterruptedException e) {
                System.err.println("InterruptedException occurred during consumption of messageFromQueue for " + consumer.getConsumerName() + " for message " + messageId);
            }
    ...
    private void retryMessage(QueueMessage queueMessage) {
        ...
        try {
            ...
            System.out.println("New message " + jsonMessage+" added to the queue, Queue size: "+getRemainingQueueCapacity());
            startConsumerThread();
        } catch (InterruptedException | IOException e) {
            System.err.println("Exception occurred during addition of message with messageId -" + queueMessage.getMessageId() + " to queue, discarding message");
        }
    }

    private void checkIfAnyConsumerHasSubscribedToMessage(String messageId, MessageSubscribersDetails messageSubscribers) throws NoSubscriberFoundException {
        if (messageSubscribers == null || messageSubscribers.getSubscribedConsumers() == null || messageSubscribers.getSubscribedConsumers().size() == 0) {
            throw new NoSubscriberFoundException("No Consumer has subscribed to message -" + messageId);
        }
    }


    @Override
    public void subscribe(Consumer consumer, String messageId, String callbackMethod, Consumer... dependentConsumers) throws InvalidDependentConsumers, ConsumerAlreadySubscribedException {
        /*
            1. Check If messageId is already present in our subscriptionList
         */
        MessageSubscribersDetails messageSubscribers = this.messagesSubscribers.get(messageId);
        if (messageSubscribers == null || messageSubscribers.getSubscribedConsumers() == null || messageSubscribers.getSubscribedConsumers().size() == 0) {
            /*
            ...
            */
            checkIfDependentConsumerListIsEmptyForNewSubscription(consumer, messageId, dependentConsumers);
            ...
            this.messagesSubscribers.put(messageId, messageSubscribers);
        } else {
             ...
             */
            checkIfDependentConsumersAreValidAndSubscribedForExistingMessage(consumer, messageId, messageSubscribers, dependentConsumers);
            /*
    ...
    private void checkIfDependentConsumerListIsEmptyForNewSubscription(Consumer consumer, String messageId, Consumer[] dependentConsumers) throws InvalidDependentConsumers {
        if (dependentConsumers != null && dependentConsumers.length != 0) {
            throw new InvalidDependentConsumers("Dependent Consumers for Consumer " + consumer.getConsumerName() + " have not yet subscribed to message - " + messageId);
        }
    }

    private void checkIfDependentConsumersAreValidAndSubscribedForExistingMessage(Consumer givenConsumer, String messageId, MessageSubscribersDetails messageSubscribers, Consumer[] dependentConsumers) throws InvalidDependentConsumers {
        ...
        if (dependentConsumerList.contains(givenConsumer)) {
            String exceptionMessage = "Subscription of message failed for Consumer "+givenConsumer.getConsumerName()+" for Message "+messageId;
            ...
            String exceptionMessage = "Subscription of message failed for Consumer "+givenConsumer.getConsumerName()+" for Message "+messageId;
            exceptionMessage += " Because, Dependent Consumers for Input Consumer - " + givenConsumer.getConsumerName() + " have not yet subscribed to message - " + messageId;
            throw new InvalidDependentConsumers(exceptionMessage);
        }
